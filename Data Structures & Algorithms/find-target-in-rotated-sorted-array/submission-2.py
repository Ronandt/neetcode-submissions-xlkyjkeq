class Solution:
    def search(self, nums: List[int], target: int) -> int:
        ...
        while right_pointer >= left_pointer:
            ...
            #if the average is greater than the left pointer
            if nums[right_pointer] > nums[average]:  # left half sorted #check whether it's in that range
                # target in left?
                if nums[average] < target <= nums[right_pointer]: #check whether it's in that range
                    left_pointer = average + 1
                else:
                    right_pointer = average - 1
            #If the average is not greater then use the left
