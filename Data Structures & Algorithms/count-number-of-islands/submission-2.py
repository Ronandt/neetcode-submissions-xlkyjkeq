class Solution:
    def numIslands(self, grid: List[List[str]]) -> int:
        ...
        visited = set() #set the length
        islands = 0

        def bfs(r, c):
            q= deque()
            visited.add((r,c))
            q.append((r,c)) #use bfs to check adj and mark them visited 
            while q:
                row, col = q.popleft()
        ...
        for r in range(rows):
            for c in range(cols):
                if grid[r][c] == "1" and (r,c) not in visited:
                    bfs(r, c) #if there's 1 then instantly check the entire adjacent (start bfs)
                    islands += 1       
        return islands
